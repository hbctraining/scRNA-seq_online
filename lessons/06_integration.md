...
<details>
  ...
  <br><b>The UMAP above can be generated by using the `seurat_phase` object from the previous lesson. For this object we have already run PCA, so the next steps would be to `RunUMAP()` and then plot, as shown in the code below.</b><br>
  <p><pre>
